...
public class SCd {
    ...
    private static final LinkedList<String> FLAGS_AND_INFO = new LinkedList<>(
            Arrays.asList(" [<flag '-s'>]/[<sc '..'>]-", "-s", ".."));

    //Parameters are [0]cd ([1]path/flag)
    ...
    public static TextComponent[] command(String[] args, Player player) {
        ...
        if (args.length == MIN_PARAM) {
            ...
            WorkingPath working_path = WorkingPath.create_path(player, "", false);
            if (!working_path.isValid()) {
                return working_path.getError();
            }
            return UiHandler.cd(working_path.getPath());
        }

        TextComponent[] result;
        WorkingPath working_path;
        //If cd is getting changed
        switch (args[1]) {
            case "..":

                working_path = WorkingPath.step_one_folder_up(WorkingPath.create_path(player, "", false));
                if (!working_path.isValid()) {
                    return working_path.getError();
                }

                UserStorage.update_or_insert(player, working_path);
                result = UiHandler.cd(working_path.getPath());
                break;

            case "-s":

                //Saving new directory
                UserStorage.remove(player);
                result = UiHandler.cd("standard directory");
                break;

            default:

                working_path = WorkingPath.create_path(player, args[1], false);
                if (!working_path.isValid()) {
                    return working_path.getError();
                }
    ...
    public static LinkedList<String> get_tab_proposals(Player player, String[] args) {
        ...
        if (args.length > MIN_PARAM && args.length <= MAX_PARAM) {
            ...
            if (tab_list.size() != 0 && tab_list.contains("-[<path>]")) {
                ...
                tab_list.addAll(0, FLAGS_AND_INFO.subList(1, FLAGS_AND_INFO.size()));
            }
        } else {
