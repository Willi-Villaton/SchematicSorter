...
public class SSave {
    ...
    private static final LinkedList<String> FLAGS_AND_INFO = new LinkedList<>(
            Arrays.asList(" [<Flag -f>]-", "-f"));

    //Parameters are [0]save [1]path
    ...
    public static TextComponent[] command(String[] args, Player player) {
        ...
        if (working_path.isValid()) {
            if (working_path.isFolder()) {
                return UiHandler.target_is_folder_error();
            } else {
                if (args.length == 2) {
                    return UiHandler.file_already_exists_error(working_path.getPath());
                }
                if (!args[2].equals("-f")) {
                    return UiHandler.file_already_exists_error(working_path.getPath());
                } else {
                    player.performCommand("/schematic save " + working_path.getPath() + " -f");
                    return null;
                }
            }
        }

        player.performCommand("/schematic save " + working_path.getPath());
        return null;
    }
